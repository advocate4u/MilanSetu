...
public sealed class InterestsController(MilanSetuDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> Send(Guid profileId, CancellationToken ct)
    {
        ...
        if (await db.Blocks.AnyAsync(x => (x.BlockerUserId == userId && x.BlockedUserId == target.UserId) || (x.BlockerUserId == target.UserId && x.BlockedUserId == userId), ct))
            return Conflict(new { message = "This connection is unavailable." });
    ...
    public async Task<IActionResult> Accept(Guid id, CancellationToken ct)
    {
        ...
        if (await db.Blocks.AnyAsync(x => (x.BlockerUserId == userId && x.BlockedUserId == interest.SenderUserId) || (x.BlockerUserId == interest.SenderUserId && x.BlockedUserId == userId), ct))
            return Conflict(new { message = "This connection is unavailable." });
