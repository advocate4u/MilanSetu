...
public sealed class ReviewerController(MilanSetuDbContext db, ReviewerAuthorizationService authorization) : ControllerBase
{
    ...
    public async Task<IActionResult> Queue(VerificationType? type, VerificationStatus? status, int page = 1, int pageSize = 25, CancellationToken ct = default)
    {
        ...
        var items = await query.OrderBy(x => x.RequestedAt).ThenBy(x => x.Id).Skip((page - 1) * pageSize).Take(pageSize)
            .Select(x => new { x.Id, x.UserId, type = x.Type.ToString(), status = x.Status.ToString(), x.RequestedAt, x.ReviewedAt, x.ReviewedByUserId, x.ClaimedByUserId, x.ClaimedAt })
            .ToListAsync(ct);
        await WriteAudit(actor.Value.UserId, "verification.queue.view", "VerificationRequest", null, null, ct);
    ...
    public async Task<IActionResult> Get(Guid id, CancellationToken ct)
    {
        var actor = await authorization.GetAuthorizedActorAsync(User, ct);
        if (actor is null) return Forbid();
        var item = await db.VerificationRequests.AsNoTracking().Where(x => x.Id == id && ReviewableTypes.Contains(x.Type))
            .Select(x => new { x.Id, x.UserId, type = x.Type.ToString(), status = x.Status.ToString(), x.RequestedAt, x.ReviewedAt, x.VerifiedAt, x.ReviewedByUserId, x.ClaimedByUserId, x.ClaimedAt, x.ReviewerNotes, email = x.User.Email })
            .SingleOrDefaultAsync(ct);
        if (item is null) return NotFound();
        await WriteAudit(actor.Value.UserId, "verification.detail.view", "VerificationRequest", id, null, ct);
        return Ok(item);
    }
    ...
    public async Task<IActionResult> Claim(Guid id, CancellationToken ct)
    {
        var actor = await authorization.GetAuthorizedActorAsync(User, ct);
        if (actor is null) return Forbid();
        var item = await db.VerificationRequests.SingleOrDefaultAsync(x => x.Id == id && ReviewableTypes.Contains(x.Type), ct);
        if (item is null) return NotFound();
        ...
        item.ClaimedByUserId = actor.Value.UserId; item.ClaimedAt = DateTimeOffset.UtcNow;
        await db.SaveChangesAsync(ct);
        await WriteAudit(actor.Value.UserId, "verification.claim", "VerificationRequest", id, null, ct);
        return Ok(new { id, claimedByUserId = actor.Value.UserId, claimedAt = item.ClaimedAt });
    }

    [HttpPost("{id:guid}/approve")]
    public async Task<IActionResult> Approve(Guid id, DecisionRequest input, CancellationToken ct)
        => await Decide(id, VerificationStatus.Verified, input, ct);

    [HttpPost("{id:guid}/reject")]
    public async Task<IActionResult> Reject(Guid id, DecisionRequest input, CancellationToken ct)
        => await Decide(id, VerificationStatus.Rejected, input, ct);
    ...
    private async Task<IActionResult> Decide(Guid id, VerificationStatus decision, DecisionRequest input, CancellationToken ct)
    {
        ...
        if (decision == VerificationStatus.Rejected && string.IsNullOrWhiteSpace(notes)) return BadRequest(new { message = "A rejection reason is required." });
        ...
        await db.SaveChangesAsync(ct);
        await WriteAudit(actor.Value.UserId, decision == VerificationStatus.Verified ? "verification.approve" : "verification.reject", "VerificationRequest", id, notes, ct);
    ...
    {
        db.AuditLogs.Add(new AuditLog { Id = Guid.NewGuid(), ActorUserId = actorUserId, Action = action, ResourceType = resourceType, ResourceId = resourceId, Metadata = notes is null ? null : JsonSerializer.Serialize(new { notes }), CreatedAt = DateTimeOffset.UtcNow });
        await db.SaveChangesAsync(ct);
    }
}
