...
public sealed class ProfileController(MilanSetuDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> GetMine(CancellationToken cancellationToken)
    {
        ...
        var profile = await db.Profiles.AsNoTracking().Include(x => x.Locations).ThenInclude(x => x.Location).Include(x => x.Preferences).SingleOrDefaultAsync(x => x.UserId == userId, cancellationToken);
