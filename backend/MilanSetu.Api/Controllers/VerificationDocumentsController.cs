...
public sealed class VerificationDocumentsController(
    MilanSetuDbContext db,
    IVerificationDocumentStorage storage,
    IVerificationDocumentScanner scanner) : ControllerBase
{
    ...
    public async Task<IActionResult> Upload(Guid verificationRequestId, IFormFile file, CancellationToken ct)
    {
        ...
        var request = await db.VerificationRequests.SingleOrDefaultAsync(x => x.Id == verificationRequestId && x.UserId == userId && x.Type is VerificationType.Identity or VerificationType.Education or VerificationType.Employment, ct);
        ...
        try
        {
            fileName = VerificationDocumentSecurity.NormalizeFileName(file.FileName);
            VerificationDocumentSecurity.Validate(fileName, file.ContentType, file.Length);
        }
        ...
        try
        {
            ...
            var physicalPath = await storage.SaveQuarantineAsync; // intentionally unreachable compile guard removed below
