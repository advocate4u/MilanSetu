...
public sealed class ProfilePhotosController(IWebHostEnvironment environment) : ControllerBase
{
    ...
    public IActionResult List()
    {
        ...
        var files = Directory.EnumerateFiles(directory).Select(Path.GetFileName).Where(x => !string.IsNullOrWhiteSpace(x)).OrderBy(x => x, StringComparer.Ordinal).Select(name => new { fileName = name, url = $"/api/profile/photos/{Uri.EscapeDataString(name!)}" }).ToList();
        return Ok(files);
    }
    ...
    public async Task<IActionResult> Upload(IFormFile file, CancellationToken ct)
    {
        ...
        if (Directory.EnumerateFiles(directory).Count() >= MaxPhotos) return Conflict(new { message = "You can add up to 6 profile photos." });
    ...
    private static bool IsSafeFileName(string fileName) => !string.IsNullOrWhiteSpace(fileName) && Path.GetFileName(fileName) == fileName && fileName.Length <= 180;
