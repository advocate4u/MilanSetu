    ...
    IVerificationCodeSender sender) : ControllerBase
{
    ...
    public async Task<IActionResult> RequestVerification(VerificationType type, CancellationToken ct)
    {
        ...
        if (type is VerificationType.Mobile or VerificationType.Email)
        {
            ...
            if (string.IsNullOrWhiteSpace(destination))
                return BadRequest(new { message = type == VerificationType.Mobile ? "Add a mobile number to your account before verification." : "Add an email address to your account before verification." });

            var alreadyVerified = type == VerificationType.Mobile ? user.IsPhoneVerified : user.IsEmailVerified;
            if (alreadyVerified)
                return Conflict(new { message = "This verification is already completed." });

            var recentChallenge = await db.VerificationChallenges
                .Where(x => x.UserId == userId && x.Type == type && x.ConsumedAt == null)
                .OrderByDescending(x => x.CreatedAt)
                .FirstOrDefaultAsync(ct);

            if (recentChallenge is not null && recentChallenge.CreatedAt > DateTimeOffset.UtcNow.Subtract(VerificationChallengeService.ResendCooldown))
                return Conflict(new { message = "Please wait before requesting another verification code." });

            if (latest is null || latest.Status != VerificationStatus.Pending || latest.RequestedAt <= DateTimeOffset.UtcNow.AddMinutes(-10))
            {
                latest = new VerificationRequest
                {
                    Id = Guid.NewGuid(),
                    UserId = userId,
                    Type = type,
            ...
            var challenge = new VerificationChallenge
            {
                ...
                CodeHash = challenges.HashCode(userId, type, code),
                CreatedAt = DateTimeOffset.UtcNow,
                ExpiresAt = VerificationChallengeService.GetExpiry(DateTimeOffset.UtcNow)
            };
            ...
            try
            {
                await sender.SendAsync(type, destination, code, ct);
            }
            catch (InvalidOperationException)
            {
                // Do not leak implementation details or the OTP. Keep the challenge unusable until a real provider is configured.
            ...
            return Accepted(new
            {
                ...
                resendAfterSeconds = (int)VerificationChallengeService.ResendCooldown.TotalSeconds,
                message = type == VerificationType.Mobile ? "A verification code was sent to your mobile number." : "A verification code was sent to your email address."
            });
        }
    ...
    {
        if (!TryGetUserId(out var userId)) return Unauthorized();
        if (type is not (VerificationType.Mobile or VerificationType.Email))
            return BadRequest(new { message = "OTP verification is available for mobile and email only." });

        var code = input.Code?.Trim() ?? string.Empty;
        if (code.Length != 6 || !code.All(char.IsDigit))
            return BadRequest(new { message = "Enter the 6-digit verification code." });
        ...
        if (challenge is null || challenges.IsExpired(challenge, DateTimeOffset.UtcNow) || challenges.HasTooManyAttempts(challenge))
            return BadRequest(new { message = "This verification code is invalid or expired. Request a new code." });
        ...
        if (request is null)
            return BadRequest(new { message = "The verification request is no longer available. Request a new code." });
        ...
        return Ok(new
        {
            ...
            message = type == VerificationType.Mobile ? "Mobile number verified successfully." : "Email address verified successfully."
        });
    }
