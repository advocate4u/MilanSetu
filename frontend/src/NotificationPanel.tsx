...
export default function NotificationPanel() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('milansetu:notifications-changed', onChanged)
  }, [refresh])
  ...
  return <>
    <button className="ms-notification-button" onClick={() => void load()} aria-label="Open notifications">
    ...
    {open && <aside className="ms-notification-panel" aria-label="Notifications">
      ...
      {items.length > 0 && <button className="ms-mark-all" onClick={() => void markAll()} disabled={unread === 0}>Mark all read</button>}
      {items.length === 0 && !error && <p>No notifications yet.</p>}
      {items.map(item => <button key={item.id} className={`ms-notification-item ${item.readAt ? '' : 'unread'}`} onClick={() => void markRead(item)}><strong>{item.title}</strong><span>{item.body}</span><small>{new Date(item.createdAt).toLocaleString()}</small></button>)}
    </aside>}
