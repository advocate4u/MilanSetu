...
export default function ProfileEditor() {
  ...
  const completeness = useMemo(() => {
    const checks = [
      ...
      Boolean(form.motherTongue.trim()), Boolean(form.bio.trim()), Boolean(form.highestQualification.trim()),
      Boolean(form.profession.trim()), Boolean(form.familyLocation.trim()), Boolean(form.interests.trim()),
      Boolean(form.foodPreference.trim()), Boolean(form.travel.trim()),
    ]
  ...
  return <section className="profile-editor" aria-label="Edit profile"><div className="profile-editor-head"><div><p className="eyebrow">MY PROFILE</p><h2>Build a profile that feels like you</h2><p>Share what helps someone understand you. Contact details and exact addresses are never collected here.</p></div></div>
    <div className="profile-completeness" aria-label={`Profile completeness ${completeness}%`}><div className="profile-completeness-row"><strong>Profile completeness</strong><span>{completeness}%</span></div><div className="profile-completeness-track"><div className="profile-completeness-value" style={{ width: `${completeness}%` }} /></div><p>Complete more fields to help other members understand your profile.</p></div>
    <form onSubmit={submit} noValidate>
    <fieldset><legend>About you</legend><div className="profile-fields"><label>Display name<input required maxLength={120} value={form.displayName} onChange={e => set('displayName', e.target.value)}/></label><label>Date of birth<input required type="date" max={new Date().toISOString().slice(0, 10)} value={form.dateOfBirth} onChange={e => set('dateOfBirth', e.target.value)}/></label><label>Gender<select value={form.gender} onChange={e => set('gender', e.target.value)}><option>Male</option><option>Female</option><option>Other</option></select></label><label>Account type<select value={form.accountType} onChange={e => set('accountType', e.target.value)}><option>Individual</option><option>Family</option><option>Assisted</option></select></label><label>Marital status<input maxLength={40} value={form.maritalStatus} onChange={e => set('maritalStatus', e.target.value)} placeholder="e.g. Never married"/></label><label>Mother tongue<input maxLength={80} value={form.motherTongue} onChange={e => set('motherTongue', e.target.value)}/></label></div><label>About me<textarea maxLength={2000} rows={5} value={form.bio} onChange={e => set('bio', e.target.value)} placeholder="Tell people about your values, interests and the life you hope to build."/></label></fieldset>
