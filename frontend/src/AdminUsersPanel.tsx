...
export default function AdminUsersPanel(){
  ...
  const load=async(term=applied)=>{setLoading(true);setMessage('');try{const me=await api('/api/auth/me');if(me.role!=='Admin'){setAllowed(false);return}setAllowed(true);const q=term?`?search=${encodeURIComponent(term)}&page=1&pageSize=50`:'?page=1&pageSize=50';const d=await api('/api/admin/users'+q);setUsers(d.items??[])}catch(e){setMessage(e instanceof Error?e.message:'Unable to load users.')}finally{setLoading(false)}}
  useEffect(()=>{void load('')},[])
