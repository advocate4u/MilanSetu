...
export async function expressInterest(profileId: string) {
  return request(`/api/interests/${profileId}`, { method: 'POST' })
}

export async function blockUser(userId: string) {
  await request(`/api/block/${userId}`, { method: 'POST' })
}

export async function unblockUser(userId: string) {
  await request(`/api/block/${userId}`, { method: 'DELETE' })
}
