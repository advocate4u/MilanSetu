...
export default function VerificationPanel() {
  ...
  const request = async (type: VerificationType) => {
    ...
    try {
      ...
      if (type === 'Mobile' || type === 'Email') {
        setOtpType(type)
        if ('resendAfterSeconds' in result) setCooldowns(current => ({ ...current, [type]: result.resendAfterSeconds }))
      }
  ...
  const verify = async (type: OtpType) => {
    setBusy(type); setError(''); setSuccess('')
    try {
      const result = await verifyVerificationCode(type, codes[type])
    ...
    } catch (e) { setError(e instanceof Error ? e.message : 'Unable to verify the code.') }
    finally { setBusy(null) }
  }

  return <section className="verification-panel" aria-label="Profile verification">
    <div className="verification-head"><div><p className="eyebrow">TRUST & VERIFICATION</p><h2>Build confidence, privately</h2><p>Mobile and email ownership can now be verified with a one-time code. Verification documents are never published as part of your public profile.</p></div></div>
    ...
    <div className="verification-grid">
      {labels.map(item => {
        ...
        const otp = item.otp && (item.type === 'Mobile' || item.type === 'Email') ? item.type : null
        return <article className="verification-card" key={item.type}>
          ...
          {verified ? <strong className="verification-badge">✓ Verified</strong> : otp ? <>
            <button type="button" className="secondary-button" disabled={busy !== null || status === 'Pending' || cooldowns[otp] > 0} onClick={() => void request(otp)}>{busy === otp ? 'Sending…' : cooldowns[otp] > 0 ? `Resend in ${cooldowns[otp]}s` : status === 'Pending' ? 'Send code again' : 'Send verification code'}</button>
            {otpType === otp && <form className="verification-otp" onSubmit={e => { e.preventDefault(); void verify(otp) }}>
              <label htmlFor={`verification-code-${otp}`}>6-digit code</label>
              <div className="verification-otp-row"><input id={`verification-code-${otp}`} inputMode="numeric" autoComplete="one-time-code" maxLength={6} pattern="[0-9]{6}" value={codes[otp]} onChange={e => setCodes(current => ({ ...current, [otp]: e.target.value.replace(/\D/g, '').slice(0, 6) }))} placeholder="000000" required /><button type="submit" className="primary-button" disabled={busy !== null || codes[otp].length !== 6}>{busy === otp ? 'Verifying…' : 'Verify'}</button></div>
              <small>The code expires after 10 minutes. Never share it with anyone.</small>
            </form>}
          </> : <button type="button" className="secondary-button" disabled={busy !== null || status === 'Pending'} onClick={() => void request(item.type)}>{busy === item.type ? 'Requesting…' : status === 'Pending' ? 'Under review' : 'Request verification'}</button>}
        </article>
      })}
    </div>
