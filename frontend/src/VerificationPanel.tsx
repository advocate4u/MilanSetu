...
export default function VerificationPanel() {
  ...
  const load = async () => { try { setItems(await getVerificationStatus()) } catch (e) { setError(e instanceof Error ? e.message : 'Unable to load verification status.') } }
  ...
  const startOtp = async (type: 'Mobile' | 'Email') => { setBusy(type); setError(''); setSuccess(''); try { await sendVerificationCode(type); setCodeFor(type); setCode(''); setSuccess(`A ${type === 'Mobile' ? 'mobile' : 'email'} verification code has been sent. It expires in 10 minutes.`); await load() } catch (e) { setError(e instanceof Error ? e.message : 'Verification delivery is unavailable.') } finally { setBusy(null) } }
  const submitCode = async () => { if (!codeFor) return; setBusy(codeFor); setError(''); setSuccess(''); try { const result = await verifyCode(codeFor, code); setSuccess(result?.message ?? 'Verification completed successfully.'); setCodeFor(null); setCode(''); await load() } catch (e) { setError(e instanceof Error ? e.message : 'The verification code is invalid or expired.') } finally { setBusy(null) } }
  const request = async (type: VerificationType) => { if (type === 'Mobile' || type === 'Email') return startOtp(type); setBusy(type); setError(''); setSuccess(''); try { const result = await requestVerification(type); setItems(current => current.map(item => item.type === type ? { ...item, status: 'Pending' } : item)); setSuccess(result?.message ?? 'Verification request submitted.') } catch (e) { setError(e instanceof Error ? e.message : 'Unable to request verification.') } finally { setBusy(null) } }
  return <section className="verification-panel" aria-label="Profile verification">
    ...
    <div className="verification-grid">{labels.map(item => { const status = statusFor(item.type); const verified = status === 'Verified'; return <article className="verification-card" key={item.type}>
