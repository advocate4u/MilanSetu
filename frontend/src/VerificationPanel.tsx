      ...
      {labels.map(item => {
        ...
        return <article className="verification-card" key={item.type}>
          ...
          {verified ? <strong className="verification-badge">✓ Verified</strong> : otp ? <>
            <button type="button" className="secondary-button" disabled={busy !== null || cooldowns[otp] > 0} onClick={() => void request(otp)}>{busy === otp ? 'Sending…' : cooldowns[otp] > 0 ? `Resend in ${cooldowns[otp]}s` : status === 'Pending' ? 'Send code again' : 'Send verification code'}</button>
